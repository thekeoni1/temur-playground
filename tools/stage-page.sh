...
#   - state-p9-net.bin.gz / state-p9-offline.bin.gz are regenerated by
...
#
# P9 rebuilt this pair on temur v0.37.0 for the launch. Only ONE pair is
# staged and committed at a time: the superseded p8 files are removed from
...
if [ -f build/state-p9-net.bin ]; then
  gzip -9 -c build/state-p9-net.bin > page/assets/state-p9-net.bin.gz
  echo "staged page/assets/state-p9-net.bin.gz (networked tier)"
else
  echo "build/state-p9-net.bin missing; networked tier will not load" >&2
fi

if [ -f build/state-p9-offline.bin ]; then
  gzip -9 -c build/state-p9-offline.bin > page/assets/state-p9-offline.bin.gz
  echo "staged page/assets/state-p9-offline.bin.gz (offline tier)"
else
  cat >&2 <<'EOF'
build/state-p9-offline.bin missing. Regenerate it with:
