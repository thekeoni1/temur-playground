...
#   - state-p10-net.bin.gz / state-p10-offline.bin.gz are regenerated by
...
#
# P10 rebuilt this pair on temur v0.38.0 for the launch. Only ONE pair is
# staged and committed at a time: the superseded p9 files are removed from
# the working tree in the same commit, and git history is what keeps them.
# Carrying both would double the ~30 MB these blobs already cost.
if [ -f build/state-p10-net.bin ]; then
  gzip -9 -c build/state-p10-net.bin > page/assets/state-p10-net.bin.gz
  echo "staged page/assets/state-p10-net.bin.gz (networked tier)"
else
  echo "build/state-p10-net.bin missing; networked tier will not load" >&2
fi

if [ -f build/state-p10-offline.bin ]; then
  gzip -9 -c build/state-p10-offline.bin > page/assets/state-p10-offline.bin.gz
  echo "staged page/assets/state-p10-offline.bin.gz (offline tier)"
else
  cat >&2 <<'EOF'
build/state-p10-offline.bin missing. Regenerate it with:
